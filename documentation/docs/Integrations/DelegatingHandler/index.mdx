...
* **Purpose**:
This `DelegatingHandler` logs the details of outgoing HTTP requests and their responses as structured logs
(using the `HttpRequestLogTemplate`/`HttpResponseLogTemplate` models from `RA.Utilities.Logging.Shared`),
including method, URI, headers, body, and status code.
* **Use Case**:
Attach it with [`WithHttpLoggingHandler`](../Extensions/DependencyInjectionExtensions.md#withhttplogginghandler)
to get a complete audit trail of every call an integration makes — invaluable for debugging external API failures.
