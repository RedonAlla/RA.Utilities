...
It is then wrapped in a `Result.Failure` object (from the `RA.Utilities.Core` package), ensuring your application doesn't crash and returns a predictable error response.
...
* **`IRequestHandler<TRequest, TResponse>`**: The most common base handler.
