...
[![NuGet version](https://img.shields.io/badge/NuGet-v10.0.9-blue?logo=nuget)](https://www.nuget.org/packages/RA.Utilities.OpenApi/10.0.9)

This release updates the package dependency from the deprecated `RA.Utilities.Api.Results` to `RA.Utilities.Api` and refreshes documentation.

<!-- truncate -->

### ⚠️ Breaking Changes

*   **Dependency updated**: `RA.Utilities.Api.Results` replaced by `RA.Utilities.Api`. The response model types remain in the same namespace (`RA.Utilities.Api.Results`). Remove any direct dependency on `RA.Utilities.Api.Results` — it is now included transitively through `RA.Utilities.Api`.

### 📝 Improvements

*   **README overhaul**: Fixed broken anchors, removed stale SwaggerGen/Swashbuckle references, documented `FluentValidationSchemaTransformer`, `EnumXmlSchemaTransformer`, and `TagOperationTransformer`, corrected config key in appsettings example, filled in empty code block.
*   **Release notes cleaned up**: Fixed "Januaryr", "117 December", and "Octomber" typos; corrected mislabeled `BearerSecurityDocumentTransformer` description in rc.2 entry.
...
*   **`BearerSecurityDocumentTransformer`**: Automatically adds a JWT Bearer security scheme to your OpenAPI document when JWT authentication is detected, enabling the "Authorize" button in Swagger UI for testing protected endpoints.
